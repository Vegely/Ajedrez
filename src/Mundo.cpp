...
void Mundo::leerTablero(const Tablero& tablero)
{
	for (int i = 0; i < 64; i++)
	{
		...
		if (tablero_anterior[i].valido || tablero_actual[i].valido &&
			tablero_anterior[i] != tablero_actual[i]) // Si alguna casilla no sigue vac�a y son distintas
		{ 
			// Movimiento normal
			if (!tablero_anterior[i].valido && tablero_actual[i].valido)
			{
			...
			{
				seleccionarLista(tablero_anterior[i].color, Pieza::tipo_t::PEON)->deleteFromCoord(tablero_anterior[i].posicion); // Borrar el pe�n
				// Volver a leer el tablero porque han pasado dos cosas a la vez: destrucci�n del pe�n y promoci�n a la nueva pieza.
				for (int j = 0; j < 64; j++)
				{
